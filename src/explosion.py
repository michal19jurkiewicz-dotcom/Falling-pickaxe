...
class Shockwave:
    """An expanding, fading ring - used to make the Nuke's explosion read as
    clearly bigger/stronger than a regular TNT blast. Exposes the same
    `particles` / `update` / `draw` interface as Explosion so it can live in
    the same `explosions` list in main.py without any special-casing.
    """

    def __init__(self, pos, max_radius, duration_ms=400, color=(255, 230, 140)):
        self.pos = pygame.Vector2(pos)
        self.max_radius = max_radius
        self.duration_ms = duration_ms
        self.color = color
        self.elapsed_ms = 0.0
        self.particles = [self]  # non-empty while active, matches Explosion's cleanup convention
    ...
    def draw(self, screen, camera):
        ...
        pygame.draw.circle(ring_surface, (*self.color, alpha), (size // 2, size // 2), radius, width)

        draw_pos = (self.pos.x - camera.offset_x - size // 2, self.pos.y - camera.offset_y - size // 2)
        screen.blit(ring_surface, draw_pos)
