...
# A dedicated RNG for terrain generation, seeded for consistent world layout
# across runs. Using a separate instance (instead of seeding the shared
# `random` module) keeps every other random event in the game - TNT timing,
# weather, pickaxe rain, etc. - unaffected and still different every run.
_terrain_rng = random.Random(SEED)
...
def generate_chunk(chunk_x, chunk_y, texture_atlas, atlas_items, space):
    ...
    for y in range(CHUNK_HEIGHT):
        ...
        for x in range(CHUNK_WIDTH):
            ...
            noise_value = _terrain_rng.uniform(-1, 1)
