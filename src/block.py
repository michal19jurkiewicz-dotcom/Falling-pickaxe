...
class Block:
    ...
    def update(self, space, hud, current_time=None, resource_multiplier=1):
        """Update block state"""
        ...
        if self.hp <= 0 and not self.destroyed:
            ...
            if self.name == "coal_ore":
                hud.amounts["coal"] += 1 * resource_multiplier  # Add to HUD amounts
            elif self.name == "iron_ore":
                hud.amounts["iron_ingot"] += 1 * resource_multiplier  # Add to HUD amounts
            elif self.name == "copper_ore":
                hud.amounts["copper_ingot"] += 1 * resource_multiplier  # Add to HUD amounts
            elif self.name == "gold_ore":
                hud.amounts["gold_ingot"] += 1 * resource_multiplier  # Add to HUD amounts
            elif self.name == "diamond_ore":
                hud.amounts["diamond"] += 1 * resource_multiplier  # Add to HUD amounts
            elif self.name == "emerald_ore":
                hud.amounts["emerald"] += 1 * resource_multiplier  # Add to HUD amounts
            elif self.name == "redstone_ore":
                hud.amounts["redstone"] += random.randint(4, 5) * resource_multiplier  # Add to HUD amounts
            elif self.name == "lapis_ore":
                hud.amounts["lapis_lazuli"] += random.randint(4, 8) * resource_multiplier  # Add to HUD amounts
